...
1.  `RandomAccessFile(String name, String mode)`: Creates a random access file stream to read from and optionally to write to the file specified by the `name` string.
2.  `RandomAccessFile(File file, String mode)`: Creates a random access file stream to read from and optionally to write to the file specified by the `File` object.

### Access Modes

1. `"r"`: Opens the file for reading only.
2. `"rw"`: Opens the file for reading and writing.
3. `"rws"`: Opens the file for reading and writing, and synchronously updates the file's content and metadata.
4. `"rwd"`: Opens the file for reading and writing, and synchronously updates the file's content.
